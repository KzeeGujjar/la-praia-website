...
- **Done (this phase — "Foundation"):** the Prisma schema for all core entities (`User` with `Role` = `SUPER_ADMIN`/`MANAGER`/`STAFF`/`CUSTOMER`, `MenuCategory`/`MenuItem`, `BusinessHours`, `Reservation`, `Order`/`OrderItem`, `RefreshToken`); JWT-based auth (`src/lib/auth/`); and read-only APIs (`/api/menu`, `/api/business/hours`) that serve from the DB.
- **Not done yet:** an admin dashboard, reservation/order-taking API endpoints and their frontend forms, and the email/SMS/WhatsApp/image-upload integrations (Resend, Twilio, Cloudinary) that those flows will eventually need. Don't assume any of these exist — check before referencing them.
- **Important split:** the live frontend (`src/app/*`, all components) still imports directly from `src/data/business.ts` / `src/data/menu.ts`, unchanged — it does **not** call `/api/menu` or `/api/business/hours` yet. `prisma/seed.ts` copies those same static files *into* the DB so the DB and the static frontend start in sync, but editing the DB after seeding will not change what the site shows until the frontend (or an admin flow) is wired to read from the DB instead. Don't assume editing a `MenuItem` row updates the live site.
